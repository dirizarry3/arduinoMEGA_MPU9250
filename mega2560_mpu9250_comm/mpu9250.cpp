...
void MPU9250::init()
{
   ...
   // wake up device
   writeI2cByte(MPU9250_ADDRESS, MPU9250_PWR_MGMT_1, 0x00); // Clear sleep mode bit (6), enable all sensors
   //wait(0.1); // Delay 100 ms for PLL to get established on x-axis gyro; should check for PLL ready interrupt

   // get stable time source
   writeI2cByte(MPU9250_ADDRESS, MPU9250_PWR_MGMT_1, 0x01);  // Set clock source to be PLL with x-axis gyroscope reference, bits 2:0 = 001

   // Configure Gyro and Accelerometer
   // Disable FSYNC and set accelerometer and gyro bandwidth to 44 and 42 Hz, respectively;
   // DLPF_CFG = bits 2:0 = 010; this sets the sample rate at 1 kHz for both
   // Maximum delay is 4.9 ms which is just over a 200 Hz maximum rate
   writeI2cByte(MPU9250_ADDRESS, MPU9250_CONFIG, 0x03);

   // Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
   writeI2cByte(MPU9250_ADDRESS, MPU9250_SMPLRT_DIV, 0x04);  // Use a 200 Hz rate; the same rate set in CONFIG above

   // Set gyroscope full scale range
   // Range selects FS_SEL and AFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions 4:3
   uint8_t c;
   readI2cByte(MPU9250_ADDRESS, MPU9250_GYRO_CONFIG, c); // get current GYRO_CONFIG register value
   // c = c & ~0xE0; // Clear self-test bits [7:5]
   c = c & ~0x02; // Clear Fchoice bits [1:0]
   c = c & ~0x18; // Clear AFS bits [4:3]
   c = c | Gscale << 3; // Set full scale range for the gyro
   // c =| 0x00; // Set Fchoice for the gyro to 11 by writing its inverse to bits 1:0 of GYRO_CONFIG
   writeI2cByte(MPU9250_ADDRESS, MPU9250_GYRO_CONFIG, c ); // Write new GYRO_CONFIG value to register

   // Set accelerometer full-scale range configuration
   readI2cByte(MPU9250_ADDRESS, MPU9250_ACCEL_CONFIG, c); // get current ACCEL_CONFIG register value
   // c = c & ~0xE0; // Clear self-test bits [7:5]
   c = c & ~0x18;  // Clear AFS bits [4:3]
   c = c | Ascale << 3; // Set full scale range for the accelerometer
   writeI2cByte(MPU9250_ADDRESS, MPU9250_ACCEL_CONFIG, c); // Write new ACCEL_CONFIG register value

   // Set accelerometer sample rate configuration
   // It is possible to get a 4 kHz sample rate from the accelerometer by choosing 1 for
   // accel_fchoice_b bit [3]; in this case the bandwidth is 1.13 kHz
   readI2cByte(MPU9250_ADDRESS, MPU9250_ACCEL_CONFIG2, c); // get current ACCEL_CONFIG2 register value
   c = c & ~0x0F; // Clear accel_fchoice_b (bit 3) and A_DLPFG (bits [2:0])
   c = c | 0x03;  // Set accelerometer rate to 1 kHz and bandwidth to 41 Hz
   writeI2cByte(MPU9250_ADDRESS, MPU9250_ACCEL_CONFIG2, c); // Write new ACCEL_CONFIG2 register value
...
void MPU9250::writeI2cByte(uint8_t addr, uint8_t subAddr, uint8_t data)
{
...
void MPU9250::readI2cByte(uint8_t addr, uint8_t subAddr, uint8_t& dataOut)
{
...
MPU9250::readAccelerometerRegisters()
{
   ...
   //Parse the data
   aData[0] = (uint16_t)(accelData[0] << 8 | accelData[1]);
   aData[1] = (uint16_t)(accelData[2] << 8 | accelData[3]);
   aData[2] = (uint16_t)(accelData[4] << 8 | accelData[5]);

   //new to resolve biases here, for now just set raw
   accelX = aData[0];
   accelY = aData[1];
   accelZ = aData[2];
...
MPU9250::readMagnometerRegisters()
{
   ...
   if(readBytes(AK8963_ADDRESS, AK8963_ST1) & 0x01) {
      ...
      mData[0] = (uint16_t)(magData[0] << 8 | magData[1]);
      mData[1] = (uint16_t)(magData[2] << 8 | magData[3]);
      mData[2] = (uint16_t)(magData[4] << 8 | magData[5]);
   }

   //new to resolve biases here, for now just set raw
   magX = mData[0] * magCalibX;
   magY = mData[1] * magCalibY;
   magZ = mData[2] * magCalibZ;
   return;
}

MPU9250::readGyroscopeRegisters()
{
   //2 bytes for each data value
   //H/L, H/L, H/L
   uint8_t gyroData[6];
...
void MPU9250::initAK8963Magnometer()
{
   unit8_t temp[3];
   ...
   magCalibOutX = (((float)temp[0]-128.0)*0.5/128.0) + 1.0;
   magCalibOutY = (((float)temp[1]-128.0)*0.5/128.0) + 1.0;
   magCalibOutZ = (((float)temp[2]-128.0)*0.5/128.0) + 1.0;
...
bool MPU9250::dataReadyToRead()
{
   uint8_t c;
   readI2cByte(MPU9250_ADDRESS, MPU9250_INT_STATUS, c)
